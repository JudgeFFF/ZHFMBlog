...
export default {
  ...
  setup() {
    ...
    const editorState = store.getters["routes/GET_EDITORSTATE"];
    const title = store.getters["articleUser/GET_TITLE"];
    const editor = store.getters["articleUser/GET_EDITOR"];
    const tag = store.getters["routes/GET_TAGSTATE"];
    const tags = store.getters["articleUser/GET_TAG"];

    if (search) {
      store
        .dispatch("articleUser/getArticleUserListByTitle", title)
        .then((res) => {
          if (res.length > 0) {
      ...
      store
        .dispatch("articleUser/getArticleUserListByUsername", editor)
        .then((res) => {
          if (res) {
